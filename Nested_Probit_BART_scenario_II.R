...
gendata = function(){
  ...
  p00 = pnorm(0.2+0.4*sin(pi*x1*x2)+0.2*(x3-0.5)**2+0.3*exp(-abs(x4))+0.6*tanh(x5*x6))
  G00 = rbinom(N,1,p00)
  
  G10 = rep(0,N)
  p10 = pnorm(-0.1+0.6*cos(pi*x1[G00==0]*x2[G00==0])+0.4*(x3[G00==0]-0.5)**2-
                0.6*exp(-abs(x4[G00==0]))+0.6*tanh(x5[G00==0]*x6[G00==0]))
  ...
  Ytmp[(G11==1)&(d==1)] = 0.3 + 0.6*cos(pi*X111[,1]*X111[,2]) + 
    0.3*(X111[,3]-0.5)**3 + 0.5*tanh(X111[,4]*X111[,5]*X111[,6])
  
  Ytmp[(G10==1)&(d==1)] = 0.1 + 0.2*sin(pi*X101[,1]*X101[,2]) +
    0.2*(X101[,3]-0.5)**3 + 0.2*tanh(X101[,4]*X101[,5]*X101[,6])
  
  Ytmp[(G11==1)&(d==0)] = -0.1 + 0.4*sin(pi*X110[,1]*X110[,2]) +
    0.1*(X110[,3]-0.5)**3 + 0.3*tanh(X110[,4]*X110[,5]*X110[,6])
  ...
  TE = mean(0.3 + 0.6*cos(pi*XG2[,1]*XG2[,2]) + 
              0.3*(XG2[,3]-0.5)**3 + 0.5*tanh(XG2[,4]*XG2[,5]*XG2[,6]) - 
              (-0.1 + 0.4*sin(pi*XG2[,1]*XG2[,2]) +
                 0.1*(XG2[,3]-0.5)**3 + 0.3*tanh(XG2[,4]*XG2[,5]*XG2[,6])))
...
combined.Gibbs = function(df=df, S){ # S is the number of iterations
  ...
  for(i in 1:S){
    ...
    IE0 = (0.3 + 0.6*cos(pi*XG2[,1]*XG2[,2]) + 0.3*(XG2[,3]-0.5)**3 + 
      0.5*tanh(XG2[,4]*XG2[,5]*XG2[,6])) - 
      (-0.1 + 0.4*sin(pi*XG2[,1]*XG2[,2]) + 0.1*(XG2[,3]-0.5)**3 + 
         0.3*tanh(XG2[,4]*XG2[,5]*XG2[,6]))
